class SeptaChatbox {
  ...
  `;
    ...
    this.sendButton.innerHTML = `<img src="https://cdn.jsdelivr.net/gh/Dwar-liberin/blob/send.png" alt="Send" style="width:24px;height:24px;">`;
  ...
  sendMessage() {
    ...
    if (message) {
      this.addMessage(`You: ${message}`, "me");
      ...
      fetch(requestUrl, {
        ...
        .then((data) => {
          if (data.result) {
            ...
            this.addMessage("Septa: " + data.result, "septa");
          }
        });
      setTimeout(() => {
        this.removeMessage(thinkingMessage); // Remove the "thinking" state
        this.addMessage(
          "Septa: hello how i can help you. this is a test message which i send you for testing purpose",
          "septa"
        );
      }, [5000]);
      this.input.value = "";
  ...
  addMessage(text, sender = "me") {
    ...
    if (sender === "septa") {
      this.typeMessage(message, text, 70);
    }
    return messageBox;
  }
}
